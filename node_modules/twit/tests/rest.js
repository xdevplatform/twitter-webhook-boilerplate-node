...
// ------------------------------------------------------------------------------------------------
  it('POST `direct_messages/events/new`', function (done) {
    var dmId
      ...
      postDm: function (next) {
        ...
        var dmParams = {
          event: {
            ...
            message_create: {
              target: {
                recipient_id: '117209856'
              },
              message_data: {
                text: 'this is a dm v2 from twit ' + helpers.generateRandomString(15)
              }
            }
          }
        }
        ...
        // post a direct message from the sender's account
        twit.post('direct_messages/events/new', dmParams, function (err, reply) {
          assert(!err, err)
          assert(reply)
          ...
          assert.equal(reply.event.message_create.message_data.text, dmParams.event.message_create.message_data.text)
  ...
  it('POST `direct_messages/welcome_messages/new`', function (done) {
    async.series({
      postWelcomeMessage: function (next) {
        var params = {
          welcome_message: {
            message_data: {
              text: 'Welcome!'
        ...
        twit.post('direct_messages/welcome_messages/new', params, function (err, reply) {
          assert(!err, err)
          assert(reply)
          exports.checkWelcomeMessage(reply.welcome_message)
          assert.equal(reply.welcome_message.message_data.text, params.welcome_message.message_data.text)
  ...
  it('GET `direct_messages/welcome_messages/show`', function (done) {
    ...
    async.series({
      postWelcomeMessage: function (next) {
        ...
        // Set welcome message
        twit.post('direct_messages/welcome_messages/new', params, function (err, reply) {
          welcomeMessageId = reply.welcome_message.id
          return next()
        })
      },
      // Make sure we get the same one
      getWelcomeMessage: function (next) {
        twit.get('direct_messages/welcome_messages/show', { id: welcomeMessageId }, function (err, reply) {
          assert(!err, err)
          exports.checkWelcomeMessage(reply.welcome_message)
          assert.equal(reply.welcome_message.id, welcomeMessageId)
          return next()
        })
      }
    }, done);
  ...
  it('DELETE `direct_messages/welcome_messages/destroy`', function (done) {
    var welcomeMessageId
    ...
    async.series({
      ...
      // Make sure we delete it the same one
      deleteWelcomeMessage: function (next) {
        twit.del('direct_messages/welcome_messages/destroy', { id: welcomeMessageId }, function (err, reply) {
          assert(!err, err)
          assert(!reply, reply)
          return next()
        })
  ...
  it('GET direct_messages/welcome_messages/list', function (done) {
    async.series({
      postWelcomeMessage: function (next) {
        var params = {
          welcome_message: {
      ...
      // Make sure we get the same one
      getWelcomeMessage: function (next) {
        twit.get('direct_messages/welcome_messages/list', { count: 1 }, function (err, reply) {
          assert(!err, err)
          exports.checkWelcomeMessage(reply.welcome_messages[0])
          return next()
        })
      }
    }, done);
  ...
  // Welcome Messages Rules

  it('POST `direct_messages/welcome_messages/rules/new`', function (done) {
    var welcomeMessageRuleId
    var welcomeMessageId

    async.series({
      // Find the id of the current rule
      getCurrentWelcomeMessageRule: function (next) {
        twit.get('direct_messages/welcome_messages/rules/list', { count: 1 }, function (err, reply) {
          if (reply.welcome_message_rules) {
            welcomeMessageRuleId = reply.welcome_message_rules[0].id
          }
          else {
            welcomeMessageRuleId = 0
          }
          return next()
        })
      },
      // Delete it because we can only have 1
      deleteCurrentWelcomeMessageRule: function (next) {
        if (welcomeMessageRuleId > 0) {
          twit.del('direct_messages/welcome_messages/rules/destroy', { id: welcomeMessageRuleId }, function (err, reply) {
            return next()
          })
        }
        return next()
      ...
      postWelcomeMessage: function (next) {
        twit.post('direct_messages/welcome_messages/new', { welcome_message: { message_data: { text: 'Welcome' } } }, function (err, reply) {
          welcomeMessageId = reply.welcome_message.id
          return next()
        })
      },
      // Create a new rule and assign the welcome message to it
      postWelcomeMessageRule: function (next) {
        var params = {
        ...
        twit.post('direct_messages/welcome_messages/rules/new', params, function (err, reply) {
          assert(!err, err)
          assert(reply)
          exports.checkWelcomeMessageRule(reply.welcome_message_rule)
          assert.equal(reply.welcome_message_rule.welcome_message_id, params.welcome_message_rule.welcome_message_id)
          return next()
        })
      }
    }, done);
  ...
  it('GET `direct_messages/welcome_messages/rules/show`', function (done) {
    var welcomeMessageRuleId

    async.series({
      getWelcomeMessageRules: function (next) {
        twit.get('direct_messages/welcome_messages/rules/list', { count: 1 }, function (err, reply) {
          welcomeMessageRuleId = reply.welcome_message_rules[0].id
          return next()
        })
      ...
      // Delete it because we can only have 1
      getCurrentWelcomeMessageRule: function (next) {
        twit.get('direct_messages/welcome_messages/rules/show', { id: welcomeMessageRuleId }, function (err, reply) {
          assert(!err, err)
          exports.checkWelcomeMessageRule(reply.welcome_message_rule)
          return next()
        })
  ...
  it.only('DELETE `direct_messages/welcome_messages/rules/destroy`', function (done) {
    var welcomeMessageRuleId
    var welcomeMessageId
    ...
    async.series({
      ...
      postWelcomeMessageRule: function (next) {
        if (welcomeMessageRuleId == 0) {
          ...
          twit.post('direct_messages/welcome_messages/rules/new', params, function (err, reply) {
            welcomeMessageRuleId = reply.welcome_message_rule.id
            return next()
          })
        }
        return next()
      },
      deletetWelcomeMessageRule: function (next) {
  ...
  it('GET `direct_messages/welcome_messages/rules/list`', function (done) {
    var welcomeMessageRuleId
    ...
    async.series({
      getWelcomeMessageRules: function (next) {
        twit.get('direct_messages/welcome_messages/rules/list', { count: 1 }, function (err, reply) {
          assert(!err, err)
          exports.checkWelcomeMessageRule(reply.welcome_message_rules[0])
          return next()
        })
      }
    }, done);
...
exports.checkWelcomeMessage = function checkWelcomeMessage(data) {
  assert.ok(data)
  assert.ok(data.id)
  assert.ok(data.message_data)
  assert.ok(data.message_data.text)
}

exports.checkWelcomeMessageRule = function checkWelcomeMessageRule(data) {
  assert.ok(data.id)
  assert.ok(data.welcome_message_id)
}
